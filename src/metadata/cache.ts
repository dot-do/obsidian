import type { TFile, CachedMetadata, EventCallback, EventRef, LinkCache, TagCache, HeadingCache, EmbedCache, BlockCache, Pos } from '../types.js'
...
export class Events {
  ...
  on<T>(event: string, callback: EventCallback<T>): EventRef {
    ...
    const listener: EventListener = { callback: callback as EventCallback<unknown>, id }
  ...
  trigger(event: string, ...args: unknown[]): void {
    ...
    for (const listener of listenersCopy) {
      try {
        ...
        ;(listener.callback as (...args: unknown[]) => void)(...args)
      } catch (e) {
...
export class MetadataCache extends Events {
  ...
  private setupVaultListeners(): void {
    ...
    this.vault.on('rename', async ({ file, oldPath }: { file: TFile; oldPath: string }) => {
  ...
  private updateLinksToDeletedFile(deletedPath: string): void {
   ...
   */
  private reindexBacklinks(oldPath: string): void {
  ...
  async indexFile(file: TFile): Promise<CachedMetadata | null> {
  ...
  getFirstLinkpathDest(linkpath: string, sourcePath: string): TFile | null {
    ...
    const normalizeForMatch = (s: string): string => s.toLowerCase().replace(/[\s_-]+/g, '')
    ...
    const sameDirMatches = files.filter(f => {
      ...
      return fDir === sourceDir && (
        f.basename === basenameToMatch ||
        normalizeForMatch(f.basename) === normalizedBasename ||
        f.path === cleanPath ||
        f.path === `${cleanPath}.md`
      )
    })
    if (sameDirMatches.length > 0) return sameDirMatches[0]

    // Then any file with matching basename (for markdown files only when no extension specified)
    if (!hasExtension) {
      const anyMatch = files.find(f =>
        (f.basename === basenameToMatch || normalizeForMatch(f.basename) === normalizedBasename) &&
        f.extension === 'md'
      )
  ...
  private addToBatch(file: TFile): void {
  ...
  private hasMetadataChanged(oldMetadata: CachedMetadata | null, newMetadata: CachedMetadata): boolean {
   ...
   */
  private updateLinkTracking(sourcePath: string, metadata: CachedMetadata): void {
    this.updateLinkGraphs(sourcePath, metadata)
  }

  private parseContent(content: string): CachedMetadata {
  ...
  private parseFrontmatter(content: string): { data: Record<string, unknown>; position: Pos } | null {
  ...
  private parseYaml(content: string): Record<string, unknown> {
    ...
    return this.parseYamlBlock(lines, 0, 0).value as Record<string, unknown>
  }

  private parseYamlBlock(lines: string[], startIndex: number, baseIndent: number): { value: Record<string, unknown>; nextIndex: number } {
  ...
  private parseYamlArray(lines: string[], startIndex: number, baseIndent: number): { value: unknown[]; nextIndex: number } {
  ...
  private extractFrontmatterLinks(data: Record<string, unknown>): Array<{ key: string; link: string; original: string; displayText?: string }> {
  ...
  private findCodeBlockRanges(content: string): Array<{ start: number; end: number }> {
  ...
  private isInCodeBlock(offset: number, codeBlockRanges: Array<{ start: number; end: number }>): boolean {
    return codeBlockRanges.some(r => offset >= r.start && offset < r.end)
  }

  private getPosition(content: string, offset: number, length: number): Pos {
  ...
  private parseLinks(content: string, codeBlockRanges: Array<{ start: number; end: number }>): LinkCache[] {
  ...
  private parseEmbeds(content: string, codeBlockRanges: Array<{ start: number; end: number }>): EmbedCache[] {
  ...
  private parseTags(content: string, codeBlockRanges: Array<{ start: number; end: number }>): TagCache[] {
    const tags: TagCache[] = []
    ...
    while ((match = regex.exec(content)) !== null) {
      ...
      if (this.isInCodeBlock(tagStart, codeBlockRanges)) continue

      // Skip if this is a URL hash - check the whole line up to the tag
      const lineStart = content.lastIndexOf('\n', tagStart) + 1
      const beforeTag = content.slice(lineStart, tagStart)
      if (beforeTag.includes('://') || beforeTag.includes('.com/') || beforeTag.includes('.org/')) continue
  ...
  private parseHeadings(content: string, codeBlockRanges: Array<{ start: number; end: number }>): HeadingCache[] {
  ...
  private parseBlocks(content: string, codeBlockRanges: Array<{ start: number; end: number }>): Record<string, BlockCache> {
