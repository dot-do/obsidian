...
import type { Backend, FileStat, TFile } from '../types.js'
  ...
  if (results.length === 0 && query.length > 0) {
    ...
    for (const file of allFiles) {
      ...
      if (tagFilters && tagFilters.length > 0) {
        ...
        const hasTag = tagFilters.some(tag =>
      ...
      const matchingLines: Array<{ line: number; text: string; positions: number[] }> = []
      let bestScore = 0
...
function getFileTags(metadata: ReturnType<MetadataCache['getFileCache']>): string[] {
  const tags: string[] = []
...
async function handleRead(
  ...
  let backlinks: Array<{ path: string; line: number; context: string }> = []
...
async function handleBacklinks(
  ...
  let allBacklinks: Array<{ path: string; position: { line: number }; context: string }> = []
  ...
  const processFile = async (targetPath: string, currentDepth: number, visited: Set<string>, targetAliases: string[]) => {
    ...
    if (currentDepth === 1 && targetAliases.length > 0) {
      ...
      for (const sourceFile of allFiles) {
        ...
        if (visited.has(sourceFile.path)) continue
        ...
        for (const link of sourceLinks) {
          ...
          const isAliasMatch = targetAliases.some(alias =>
            alias.toLowerCase() === linkTarget.toLowerCase()
          )
  ...
  // Deduplicate by path
  const uniqueBacklinks = new Map<string, { path: string; position: { line: number }; context: string }>()
