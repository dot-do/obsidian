import type { Backend, FileStat, EventCallback, EventRef } from '../types.js'
...
export class MemoryBackend implements Backend {
  ...
  private eventListeners = new Map<string, Set<{ callback: BackendEventCallback; ref: EventRef }>>()
  ...
  async read(path: string): Promise<string> {
    ...
    if (content instanceof ArrayBuffer) throw new Error(`File is binary: ${path}`)
    return content
  }
  ...
  async rename(from: string, to: string): Promise<void> {
    ...
    if (stat) this.stats.set(to, { ...stat, mtime: Date.now() })
  ...
  on(event: string, callback: BackendEventCallback): EventRef {
    ...
    const ref: EventRef = { unsubscribe: () => this.off(event, ref) }
