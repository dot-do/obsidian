...
export class ObsidianClient {
   ...
   */
  getRecentNotesSync(limit = 10): TFile[] {
    this.ensureInitialized()
    const files = this.vault.getMarkdownFiles()
   ...
   */
  async getRecentNotesWithContent(limit = 10): Promise<NoteWithContent[]> {
    this.ensureInitialized()
   ...
   */
  getNotesByFolder(folder: string, recursive = true): TFile[] {
    this.ensureInitialized()
   ...
   */
  getNotesByTags(tags: string[], requireAll = false): TFile[] {
    this.ensureInitialized()
   ...
   * @returns Promise resolving to array of NoteWithContent objects
   */
  async getNotesWithContent(options: NoteFilterOptions): Promise<NoteWithContent[]> {
    this.ensureInitialized()
   ...
   * @throws Error if file not found
   */
  async deleteNote(path: string): Promise<this> {
    this.ensureInitialized()
    this.ensureNotDisposed()
   ...
   */
  async renameNote(oldPath: string, newPath: string): Promise<this> {
    this.ensureInitialized()
    this.ensureNotDisposed()
