 ...
 */
export const vaultSearchTool: McpToolDefinition = {
  name: 'vault_search',
  description:
    'Search for notes in the vault by content or filename. Returns matching notes with relevance scores and contextual snippets. Supports filtering by tags.',
  inputSchema: {
    type: 'object',
    properties: {
      query: {
 ...
 */
export const vaultListTool: McpToolDefinition = {
  name: 'vault_list',
  description:
    'List all markdown files in the vault or a specific folder. Returns file paths, names, and basic metadata like modification time and size.',
  inputSchema: {
    type: 'object',
    properties: {
      folder: {
 ...
 */
export const noteReadTool: McpToolDefinition = {
  name: 'note_read',
  description:
    'Read the content of a note at the specified path. Returns the full content along with parsed metadata including frontmatter, headings, and links.',
  inputSchema: {
    type: 'object',
    properties: {
      path: {
 ...
 */
export const noteCreateTool: McpToolDefinition = {
  name: 'note_create',
  description:
    'Create a new note at the specified path with the given content. Optionally include frontmatter metadata. Fails if a note already exists at the path.',
  inputSchema: {
    type: 'object',
    properties: {
      path: {
 ...
 */
export const noteUpdateTool: McpToolDefinition = {
  name: 'note_update',
  description:
    'Update the entire content of an existing note, replacing its current content completely. Use note_append for adding content without overwriting.',
  inputSchema: {
    type: 'object',
    properties: {
      path: {
 ...
 */
export const noteAppendTool: McpToolDefinition = {
  name: 'note_append',
  description:
    'Append content to an existing note at a specified position. Can append at the end, start, or after frontmatter.',
  inputSchema: {
    type: 'object',
    properties: {
      path: {
 ...
 */
export const frontmatterUpdateTool: McpToolDefinition = {
  name: 'frontmatter_update',
  description:
    'Update the frontmatter (YAML metadata) of a note. Can merge with existing frontmatter or replace it entirely.',
  inputSchema: {
    type: 'object',
    properties: {
      path: {
 ...
 */
export const graphBacklinksTool: McpToolDefinition = {
  name: 'graph_backlinks',
  description:
    'Get all backlinks pointing to a note. Returns a list of notes that contain links to the specified note, with optional context showing where the link appears.',
  inputSchema: {
    type: 'object',
    properties: {
      path: {
 ...
 */
export const graphForwardLinksTool: McpToolDefinition = {
  name: 'graph_forward_links',
  description:
    'Get all forward links from a note. Returns notes that the specified note links to, including link counts and optionally unresolved (broken) links.',
  inputSchema: {
    type: 'object',
    properties: {
      path: {
 ...
 */
export const graphNeighborsTool: McpToolDefinition = {
  name: 'graph_neighbors',
  description:
    'Get neighbor notes in the knowledge graph within a specified depth. Traverses both incoming and outgoing links to find related notes.',
  inputSchema: {
    type: 'object',
    properties: {
      path: {
 ...
 */
export const vaultContextTool: McpToolDefinition = {
  name: 'vault_context',
  description:
    'Generate context from the vault based on a scope. Useful for gathering relevant content for AI context windows. Supports scopes like "all", "folder:path", "tag:name", "recent:7d", "linked:note.md".',
  inputSchema: {
    type: 'object',
    properties: {
      scope: {
        type: 'string',
        description:
          'The scope to gather context from. Options: "all" (entire vault), "folder:path" (specific folder), "tag:name" (notes with tag), "recent:Nd" (notes modified in last N days), "linked:path" (notes linked to/from a note)',
      },
      maxTokens: {
        type: 'integer',
 ...
 */
export const obsidianTools: McpToolDefinition[] = [
 ...
 */
export function getReadOnlyTools(): McpToolDefinition[] {
 ...
 */
export function getWriteTools(): McpToolDefinition[] {
  return obsidianTools.filter(tool => tool.annotations?.readOnlyHint !== true)
}
 ...
 */
export function getDestructiveTools(): McpToolDefinition[] {
