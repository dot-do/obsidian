    ...
    getBacklinksWithContext(path) {
        ...
        for (const sourcePath of sourcePaths) {
            ...
            if (sourceFile && metadata?.links) {
                const relevantLinks = metadata.links.filter(l => this.cache.getFirstLinkpathDest(l.link, sourcePath)?.path === path);
