 ...
 * - After a digit
 */
function isAtWordBoundary(text, pos) {
    if (pos === 0)
        return true;
 ...
 */
export function prepareSimpleSearch(query) {
 ...
 */
function findFuzzyMatch(queryLower, textLower, originalText) {
 ...
 */
function findBestMatchPositions(queryLower, textLower, originalText) {
    ...
    while (queryIdx < queryLower.length && textIdx < textLower.length) {
        ...
        const bestCandidate = pickBestCandidate(candidates, positions, textLower, originalText);
 ...
 */
function canMatch(queryLower, textLower) {
    let queryIdx = 0;
 ...
 */
function canMatchFrom(queryLower, queryStart, textLower, textStart) {
 ...
 */
function pickBestCandidate(candidates, prevPositions, textLower, originalText) {
 ...
 */
function isCamelCaseBoundary(text, pos) {
    if (pos === 0)
        return true;
    const currChar = text[pos];
    const prevChar = text[pos - 1];
 ...
 */
function calculateScore(queryLower, textLower, originalText, positions) {
    let score = 0;
