 ...
 */
export function resolveVaultPath(flags) {
 ...
 */
export function routeToCommand(command, args, flags) {
    throw new Error('Not implemented');
}
/**
        ...
        try {
            ...
            if (options.json) {
                if (options.count) {
                    ...
                    console.log(JSON.stringify(sortedTags.map(([tag]) => tag), null, 2));
                }
            }
            else {
                if (options.count) {
...
export async function main(argv) {
    ...
    if (parsed.flags.help) {
        console.log(`obsidian.do - Obsidian-compatible API for headless vault operations
...
Commands:
  ...
  backlinks <path>  Show backlinks for a note
  list [folder]     List files in the vault
  tags              List all tags in the vault
  serve             Start HTTP server for REST API access
  mcp               Start MCP server mode
...
`);
    ...
    switch (command) {
        case 'serve': {
            const { main: serveMain } = await import('./serve.js');
            return serveMain(parsed.args, parsed.flags);
        }
        case 'mcp': {
            // Resolve vault path
            const vaultPath = typeof parsed.flags.vault === 'string'
                ? parsed.flags.vault
        ...
        case 'search': {
            ...
            try {
                ...
                let filteredResults = results;
                if (typeof parsed.flags.tags === 'string') {
                    const tagFilters = parsed.flags.tags.split(',').map(t => t.trim().replace(/^#/, '').toLowerCase());
                    filteredResults = results.filter(r => {
                        const metadata = client.metadataCache.getFileCache(r.file);
                        const fileTags = getFileTags(metadata);
                        return tagFilters.some(tag => fileTags.some(ft => ft.toLowerCase() === tag));
        ...
        case 'read': {
            ...
            try {
                ...
                if (parsed.flags.json) {
                    ...
                    console.log(note.content);
                    if (parsed.flags.backlinks && note.backlinks.length > 0) {
                        console.log('\n--- Backlinks ---');
                        for (const bl of note.backlinks) {
                            console.log(`- ${bl.path}`);
                        }
        ...
        case 'create': {
            ...
            try {
                const client = await createClient(vaultPath);
                const frontmatter = typeof parsed.flags.tags === 'string'
                    ? { tags: parsed.flags.tags.split(',').map(t => t.trim().replace(/^#/, '')) }
                    : undefined;
                const content = typeof parsed.flags.content === 'string' ? parsed.flags.content : '';
                await client.createNote(notePath, content, frontmatter);
                if (parsed.flags.json) {
                    console.log(JSON.stringify({ path: notePath, created: true }, null, 2));
                }
                else {
