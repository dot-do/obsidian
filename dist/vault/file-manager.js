...
export class FileManager {
    ...
    listFilesByExtension(extension) {
        ...
        const normalizedExtensions = extensions.map(ext => ext.startsWith('.') ? ext.substring(1).toLowerCase() : ext.toLowerCase());
        return this.vault.getFiles().filter(file => normalizedExtensions.includes(file.extension.toLowerCase()));
    }
    /**
     ...
     */
    watchFileChanges(callback, options) {
        const refs = [];
        const shouldNotify = (file) => {
     ...
     */
    getLinkPath(targetPath, sourcePath) {
        ...
        const filesStartingWithBasename = allFiles.filter(f => f.extension === 'md' &&
     ...
     */
    getRelativePath(targetPath, sourcePath) {
        const normalizedTarget = this.normalizePath(targetPath);
        const normalizedSource = this.normalizePath(sourcePath);
    ...
    getBacklinks(file) {
        ...
        for (const mdFile of allMdFiles) {
            if (mdFile.path === targetPath)
                continue; // Skip self
            const metadata = this.cache.getFileCache(mdFile);
            if (!metadata?.links && !metadata?.embeds)
                continue;
            const links = [];
     ...
     * @private
     */
    linkResolvesToFile(linkTarget, sourceDir, targetBasename, targetPath, targetPathWithoutExt, targetDir, allMdFiles) {
     ...
     */
    updateLinks(content, oldPath, newPath) {
